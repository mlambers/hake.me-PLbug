...
function cancerLancerBug.FindTargetForFakeRash(radius)
	...
	for _,npc in pairs(entity) do
		if npc and Entity.IsAlive(npc) and not Entity.IsDormant(npc) and not NPC.HasState(npc,Enum.ModifierState.MODIFIER_STATE_NO_HEALTH_BAR) and not NPC.IsStructure(npc) then
